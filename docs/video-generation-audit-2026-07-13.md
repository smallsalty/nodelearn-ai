...
| J | Fallback | 质量失败可重写一次；无严格 repair 和确定性 storyboard fallback | 策略/Narrative 失败使用确定性版本；通用 storyboard 只 repair 一次，仍失败使用五段 fallback；哈希按目标时长在六个核心场景上增加最多四个确定性教学场景 |
...
- 公共 `targetDurationSeconds` 既是规划意图也是发布门禁；最终媒体时长仍完全由真实逐场景音频决定，但 estimated timeline 和真实 timeline 都必须落在目标时长 ±15% 内。
