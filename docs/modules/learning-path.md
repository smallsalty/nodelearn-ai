...
- 每个任务展示 `dueAt` 建议完成时间，并为 `learn/practice/review/project` 全部任务提供四种匹配任务类型的学习工具、中文用途说明、可直接复制的中文提示词和工具入口。
- 四类任务都包含“视频讲解”工具卡，并按学习、练习、复习和项目语义生成不同提示词；工具入口只打开 `action=knowledge_video` 页面，不会因为学习路径展示或点击而自动生成视频。
