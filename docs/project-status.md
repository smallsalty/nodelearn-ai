...
- 2026-07-20 完成首页学生画像摘要最小中文化：仅调整 `HomePage.vue` 展示层，基于结构化画像字段生成中文摘要，将课程编号、薄弱节点编号、难度、认知风格、练习偏好和资源偏好转换为中文，并对重复知识点名称保序去重；节点名称解析失败时显示中文占位，不回显内部 ID。本次未修改接口、契约、后端或数据库；按用户要求未执行测试、构建或浏览器验收。
