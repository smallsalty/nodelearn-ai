...
- 2026-07-20 修复数字人直播 `manifestLoadError` 并部署腾讯云：确认 `www.smalllightsalty.top` 页面收到的直播地址被固定为根域名，浏览器跨域请求未到达服务器；实时 `DigitalHumanChatResult.videoUrl` 与 `DigitalHumanLiveSessionResult.videoUrl` 现统一使用 `/storage/digital-human-live/{sessionId}/index.m3u8` 同源相对路径，持久生成资源的绝对公网地址语义不变。数字人/API 定向测试 `14 passed`、后端完整测试 `218 passed, 2 skipped`、Python 编译与服务器 Compose 校验通过；服务器精确部署提交 `eb6cbb5` 并仅替换 backend，旧镜像保留为 `nodelearn-ai-backend:pre-manifest-fix-20260720T061439Z`，frontend、PostgreSQL、Judge0、Caddy 和数据卷保持原容器。Playwright 经 SSH 隧道执行一次短时真实问答，API 返回相对地址，manifest 与连续 HLS 分片全部 200，视频 `readyState=4`、非暂停且播放时间持续推进，控制台 0 error/0 warning；根域名与 `www` 经 Caddy 访问同一 manifest 均为 HTTP 200 和 `application/vnd.apple.mpegurl`。主动 stop 返回 200，最终 FFmpeg 和 SSH 隧道均为 0，两个域名健康接口为 200，backend 近十分钟无错误日志。
...
- 2026-07-19 演示真实取景验收：经腾讯云 SSH 隧道以 `demo_student` 完成首页、知识图谱、画像、六项学习路径、5.1 栈真实 DeepSeek 课程问答、6.1 哈希表真实思维导图生成、视频入口、数字人入口、练习入口和 PostgreSQL 笔记保存；新思维导图任务 `resource_task_c94f3efe5c07`、资源 `resource_mind_map_f4c51c35f2e8` 为 `success/passed`，笔记 `note_cdee63cd8694` 显示“已与服务端同步”，浏览器控制台 0 error/0 warning。审片样片复用 2026-07-12 已验收的 120.512 秒 `success/passed` 哈希教学视频、2026-07-17 实时数字人 `PASS_REAL` 证据和 2026-07-15 Judge0 AC 证据；未使用两条约 62.7 秒失败视频，也未重新发起付费视频或数字人会话。
- 2026-07-19 录制环境复测：腾讯云后端、PostgreSQL、Redis、课程材料检索、LLM、数字人对话和在线虚拟人健康；旧讯飞 TTS 健康项仍为 `error`，当前知识视频使用豆包 TTS，不受该项影响。公网 `https://smalllightsalty.top` 当前仍连接重置，因此本轮按预案使用腾讯云 SSH 隧道取景并要求成片裁去地址栏；正式公网录制仍受既有 ICP 备案/接入备案阻塞。
...
- 2026-07-19 完成“视频讲解”学习工具与推荐整合：左侧学习工具新增按 `path + action=knowledge_video` 精确高亮的正式入口，资源页支持视频参数预选、成功/审核通过资源过滤和 `nodeId + resourceId` 历史视频深链；课程正文、知识图谱和知识节点正文兼容页均在思维导图旁补齐视频入口，四类学习路径任务均提供带对应中文提示词的视频讲解工具卡。
- 2026-07-19 完成知识视频推荐补齐：成功、审核通过且具有文件地址的 `knowledge_video` 会幂等创建现有推荐与推送记录，推荐读取可补齐历史合格视频、按时间倒序并排除失败/未审核/无文件视频；首页和学习浮窗推荐支持键盘访问、已查看标记和共享深链跳转，读取推荐或展示学习路径不会自动触发付费视频生成。本次复用现有接口、字段、枚举和数据库表，公共契约未变。
- 2026-07-19 视频讲解入口回归结果：后端完整测试 `214 passed, 2 skipped`，前端生产构建、`video-renderer` TypeScript 检查、页面/组件无直接 `fetch` / `axios` 和 `git diff --check` 通过；Playwright 在 mock 后端验证正式导航、视频页预选、学习路径视频卡及跳转，全程没有视频生成请求，375/768/1024/1440px 均无横向溢出，控制台 0 error/0 warning。
- 2026-07-19 按页面批注精简前端辅助文案：资源中心与数字人解答仅保留主要功能描述，画像对话提示改为“请输入学习背景。”，知识图谱删除固定布局说明；未修改接口、字段、审计流程或业务交互。
...
- 2026-07-19 哈希表 120 秒真实视频最终交付仍阻塞：本次授权的首次调用和一次完整付费重试都已消耗，第二次因残留旧 Uvicorn 子进程未加载时长修复而再次产出约 62.7 秒短片。代码和非付费回归已修复通过，但继续生成合格新 MP4 需要用户重新授权一次真实 DeepSeek 与豆包 TTS 调用。
