...
> 文件用途：本文件用于约束 NodeLearn AI: Multi-Agent Personalized Learning System 的前后端接口、核心变量、枚举值、数据表字段和 Codex 开发规则。  
...
第 8 步：逐个补充真实业务逻辑
第 9 步：增加 contract tests，防止接口偏移
```
...
6. 未实现功能可以返回 mock 数据，但接口路径和字段必须保持一致。
...
开发早期允许使用 mock 数据，但必须满足：

```text
1. mock 字段必须与本文件接口字段完全一致。
2. mock 返回必须包裹 ApiResponse<T>。
3. mock 接口路径必须与正式接口路径一致。
4. mock 枚举值必须来自本文件定义。
5. mock 数据不得新增临时字段。
```
...
6. 后端不得新增未定义路由。
7. 未实现功能只能保留空接口、TODO 或 mock 返回，不允许自造替代接口。
8. 如果发现说明书中缺少必要接口或变量，停止编码，输出：CONTRACT_MISSING: 缺少【名称】定义。
9. 每次提交代码后，必须运行 contract tests，确保接口和变量没有偏离说明书。
10. safety_agent 或 audit 接口未通过时，不允许把生成资源标记为可用。
```

---

# 28. Contract Tests 要求
